...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
    	agentMap.put("OFFLINEUSER", new AgentDO("OFFLINEUSER", "Kaustubh Ayare"));
        agentMap.put("DEMOUSER", new AgentDO("DEMOUSER", "Prathmesh Pawar"));
        agentMap.put("AGENT1", new AgentDO("AGENT1", "Deepak Kumavat"));
        agentMap.put("AG0001", new AgentDO("AG0001", "Kunj Shah"));
        agentMap.put("AGENT2", new AgentDO("AGENT2", "Asmi Purohit"));
        while (true) {
    ...
    public static void searchPolicy() {
        ...
        System.out.println("3. By AgentId");
    ...
    public static void searchByAgentId(String agentId)
    {
    	for (Map.Entry<String, LinkedHashMap<String,basePolicyDO>> partyEntry : policyMap.entrySet())
    	{
    		LinkedHashMap<String,basePolicyDO> policies = partyEntry.getValue();
    		for (basePolicyDO policy : policies.values())
    		{
    			...
    			{
    				termInsuranceDO temp=(termInsuranceDO) policy;
    				if(temp.getAgentId().equalsIgnoreCase(agentId))
    				System.out.println(policy);
    			}
    			else if(policy instanceof termHealthInsuranceDO)
    			{   				
    				termHealthInsuranceDO temp=(termHealthInsuranceDO) policy;
    				if(temp.getAgentId().equalsIgnoreCase(agentId))
    				System.out.println(policy);
    			}
    		}
